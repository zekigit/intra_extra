...
def source_loc(evoked, cov, fwd, subj, img_type, study_path, plot=True):
...
def dipole_source_loc(evoked, cov,  fs_subj, study_path):
    evo_crop = evoked.copy().crop(-0.003, 0.003)
    subj = fs_subj.strip('_an') if fs_subj.find('_an') > 0 else fs_subj
    img_type = 'anony' if fs_subj.find('_an') > 0 else 'orig'

    ori_to_an_fname = op.join(study_path, 'freesurfer_subjects', subj, 'mri', 'ori_to_an_trans.lta')
    ...
    dip.plot_locations(trans_fname, fs_subj, subjects_dir, mode='orthoview', coord_frame='mri', ax=ax, show_all=False,
                       idx='gof')
...
if __name__ == '__main__':
    ...
    for cond_fname in conds[:2]:
        ...
        dist_dis = source_loc(evoked, cov, fwd, subj, img_type, study_path)
        dist_dip_err = dipole_source_loc(cond_fname, subj, img_type, study_path)

        cond = eeg_epo.info['description']
        stim_params = get_stim_params(cond)

        save_results(subj, study_path, stim_params, evoked, dist_dis, dist_dip)
